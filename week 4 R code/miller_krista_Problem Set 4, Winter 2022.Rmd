...
#model.main <- aov(sim_tot_fat~flour_type_factor+fat_type_factor, data= doughnuts.factorial) # Complete this line

model.main <- lm(sim_tot_fat~flour_type_factor+fat_type_factor, data= doughnuts.factorial) 
...
model.interaction <-  lm(sim_tot_fat~flour_type_factor*fat_type_factor, data= doughnuts.factorial)# Complete this line

#model.interaction <-  aov(sim_tot_fat~flour_type_factor*fat_type_factor, data= doughnuts.factorial)
# Use the summary() function to display your results!
...
Upon occasion, using the select() function will cause an error because there is also a base R function named select(). If you encounter an error and you believe that this is the cause, R allows you to add the library name in front of the function to clarify the source of the function. For example: dpylr::select(fish, fisherman_factor, fishpart_factor, age, restime, height, weight, fishmlwk, logTotHg). If you encounter an error running the above code chunk, try adding dpylr:: in front of the select() function.
...
lm.null<- lm(logTotHg~1, data= fish.auto)


fwd.model<- step(lm.null, direction='forward', scope= ~ fisherman_factor + age + restime + height+ weight + fishmlwk + fishpart_factor, trace=1)
...
lm.full<- lm(logTotHg ~ ., data=fish.auto)
bkwd.model<- step(lm.full, direction ="backward", trace=1)
...
best<- regsubsets(logTotHg~., data=fish.auto, method= "exhaustive", nvmax=9, nbest=1)
...
Display the model selected using best subsets selection with BIC values. Note: you do not need to "scrape" from the results of using regsubsets() to earn full credit here - you just need to display the model selected using that process, which you can specify manually if you so choose. 

```{r}

# Display the model selected by best subsets selection 

bestsubsets.model = lm(logTotHg~weight+fishpart_factor, data=fish) 
...
Your answer here: weight, fishpart_factor1, fishpart_factor2, fishpart_factor3 

Question B: Which predictors were included in the model you chose using backward selection?

Your answer here:  fishpart_factor1, fishpart_factor2, fishpart_factor3, weight 
